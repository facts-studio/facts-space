...
function Pill({ active, onClick, href, icon, children }) {
  const inner = (
    <>
  ...
  if (href) {
    return <Link href={href} className={cn(PILL, IDLE)}>{inner}</Link>;
  }
  return (
...
export default function QuickLinks({ mode = "inicio", onSelect, presence = [], showStatus = true, showTimeline = true, className = "" }) {
  ...
  return (
    <nav aria-label="Accesos directos" className={cn("flex flex-wrap items-center gap-2", className)}>
      ...
      {showTimeline && <Pill href="/timeline" icon="timeline">Timeline</Pill>}
